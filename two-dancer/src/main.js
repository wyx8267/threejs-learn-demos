...
renderer.domElement.addEventListener('click', (e) => {
  ...
  const intersects = rayCaster.intersectObjects(stage.children);
  const set = new Set();
  ...
  const danceArr = [...set].slice(0, 1)
  outlinePass.selectedObjects = set.size ? danceArr : []

  if (danceArr.length) {
    const isDancer1 = danceArr[0].name === 'dancer1';
    // if (isDancer1){
    ...
    const tween = new Tween(camera.position)
      .to(isDancer1 ? { x: 24, y: 955, z: -500 } : { x: 42, y: 1008, z: 479 }, 2000)
      .repeat(0)
